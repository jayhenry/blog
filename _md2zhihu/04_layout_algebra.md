...
$$
(\text{M}, \text{N}) \xrightarrow{\text{zipped_divide by }(\text{TileM}, \text{TileN})}
...
<img src="https://www.zhihu.com/equation?tex=%2816%2C16%29%3A%2816%2C1%29%5C%5C" alt="(16,16):(16,1)\\" class="ee_img tr_noresize" eeimg="1">

`zipped_divide` 后得到：

<img src="https://www.zhihu.com/equation?tex=%28%281%2C8%29%2C%2816%2C2%29%29%3A%28%280%2C1%29%2C%2816%2C8%29%29%5C%5C" alt="((1,8),(16,2)):((0,1),(16,8))\\" class="ee_img tr_noresize" eeimg="1">
...
![zipped_divide input layout](https://gitee.com/henryzhao28/blog/raw/main-md2zhihu-asset/04_layout_algebra/213737d60a46524a-04_zipped_divide_layout.jpg)

zipped_divide result layout:

![zipped_divide result layout](https://gitee.com/henryzhao28/blog/raw/main-md2zhihu-asset/04_layout_algebra/27ba050214e9b4d9-04_zipped_divide_result.jpg)
...
$$
(\text{M}, \text{N}) \xrightarrow{\text{logical_divide by }(\text{TileM}, \text{TileN})}
...
下面把一个 <img src="https://www.zhihu.com/equation?tex=16%20%5Ctimes%2016" alt="16 \times 16" class="ee_img tr_noresize" eeimg="1"> 的行主序矩阵，沿列方向按 8 个元素一组做逻辑分块：
...
<img src="https://www.zhihu.com/equation?tex=%28%281%2C16%29%2C%288%2C2%29%29%3A%28%280%2C16%29%2C%281%2C8%29%29%5C%5C" alt="((1,16),(8,2)):((0,16),(1,8))\\" class="ee_img tr_noresize" eeimg="1">

这里第一维的 tile 大小是 1，所以行方向实际上没有被切分；第二维按 8 列分块，因此列方向被拆成“块内 8 列”和“共 2 个块”两层。

2D logical_divide input layout:

![2D logical_divide input layout](https://gitee.com/henryzhao28/blog/raw/main-md2zhihu-asset/04_layout_algebra/213737d60a46524a-04_logical_divide_2d_layout.jpg)

2D logical_divide result layout: 由于 `cute-viz` 将最左边坐标 `(1,16)`解读为块内坐标，将 `(8,2)`解读为块坐标，所以不能得到 `(1,8)`为Tile的理想可视化结果。

![2D logical_divide result layout](https://gitee.com/henryzhao28/blog/raw/main-md2zhihu-asset/04_layout_algebra/50875cdd463d3c7e-04_logical_divide_2d_result.jpg)
...
$$
(\text{M}, \text{N}) \xrightarrow{\text{tiled_divide by }(\text{TileM}, \text{TileN})}
((\text{TileM}, \text{TileN}), \text{RestM}, \text{RestN})
...
<img src="https://www.zhihu.com/equation?tex=%28%281%2C8%29%2C16%2C2%29%3A%28%280%2C1%29%2C16%2C8%29%5C%5C" alt="((1,8),16,2):((0,1),16,8)\\" class="ee_img tr_noresize" eeimg="1">

和 `zipped_divide` 相比，这里 tile 内形状仍然是 `(1,8)`，但外层的两个剩余坐标 `16` 与 `2` 不再额外打包成一个 mode，而是直接顺次排在后面。

tiled_divide input layout:

![tiled_divide input layout](https://gitee.com/henryzhao28/blog/raw/main-md2zhihu-asset/04_layout_algebra/213737d60a46524a-04_tiled_divide_layout.jpg)

tiled_divide result layout: 由于`display_layout`只能图示2维的内容。当layout维度超过2维时，只图示最后2维的内容，前面其余的维度会遍历依次排开。所以也不能理想地展示tile情况。

![tiled_divide result layout](https://gitee.com/henryzhao28/blog/raw/main-md2zhihu-asset/04_layout_algebra/387a32ce45bbe0da-04_tiled_divide_result.jpg)
