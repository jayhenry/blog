# CuTe：
`Swizzle`
 如何重排 Layout 以缓解 Shared Memory Bank Conflict
...
1.  再看一个更接近实际使用的 shared memory layout，说明 `swizzle_bits` 是怎么根据一次 copy 的位宽推出来的。
...
<img src="https://www.zhihu.com/equation?tex=%288%2C8%29%3A%288%2C1%29%5C%5C" alt="(8,8):(8,1)\\" class="ee_img tr_noresize" eeimg="1">

这表示逻辑形状是 `(8, 8)`，步长是 `(8, 1)`，也就是最常见的按行连续存储。下面这张图展示了这个基础 layout 的编号方式。

![The base `8x8` layout is a plain row-major mapping before any swizzle is applied.](https://gitee.com/henryzhao28/blog/raw/main-md2zhihu-asset/07_swizzle/2a40ed88699cce00-07_swizzle_base_8x8.jpg)
...
<img src="https://www.zhihu.com/equation?tex=%5Ctext%7Blayout%7D%3A%20%5Ctext%7Bcoord%7D%20%5Cmapsto%20%5Ctext%7Boffset%7D%5C%5C" alt="\text{layout}: \text{coord} \mapsto \text{offset}\\" class="ee_img tr_noresize" eeimg="1">

那么 swizzle 可以写成：

<img src="https://www.zhihu.com/equation?tex=%5Ctext%7Bswizzle%7D%3A%20%5Ctext%7Boffset%7D%20%5Cmapsto%20%5Ctext%7Boffset%7D%27%5C%5C" alt="\text{swizzle}: \text{offset} \mapsto \text{offset}'\\" class="ee_img tr_noresize" eeimg="1">

两者组合之后，得到的就是复合函数：

<img src="https://www.zhihu.com/equation?tex=%28%5Ctext%7Bswizzle%7D%20%5Ccirc%20%5Ctext%7Blayout%7D%29%28%5Ctext%7Bcoord%7D%29%3D%20%5Ctext%7Bswizzle%7D%28%5Ctext%7Blayout%7D%28%5Ctext%7Bcoord%7D%29%29%5C%5C" alt="(\text{swizzle} \circ \text{layout})(\text{coord})= \text{swizzle}(\text{layout}(\text{coord}))\\" class="ee_img tr_noresize" eeimg="1">
...
<img src="https://www.zhihu.com/equation?tex=%5Ctext%7BSwizzle%7D%20%3D%20S%3C3%2C0%2C3%5Cgt%5C%5C" alt="\text{Swizzle} = S<3,0,3\gt\\" class="ee_img tr_noresize" eeimg="1">

<img src="https://www.zhihu.com/equation?tex=%5Ctext%7BSwizzled%20layout%7D%20%3D%20S%3C3%2C0%2C3%5Cgt%20%5Ccirc%200%20%5Ccirc%20%288%2C8%29%3A%288%2C1%29%5C%5C" alt="\text{Swizzled layout} = S<3,0,3\gt \circ 0 \circ (8,8):(8,1)\\" class="ee_img tr_noresize" eeimg="1">

第一张图展示了 swizzle 之后的重排关系。可以把它理解成：原来连续落在同一列上的元素，被重新分散到不同列中。

![The `Swizzle<3,0,3>` view shows how a regular `8x8` layout is remapped across columns.](https://gitee.com/henryzhao28/blog/raw/main-md2zhihu-asset/07_swizzle/9f001d4390d8c39e-07_swizzle_swizzled_8x8.jpg)
...
<img src="https://www.zhihu.com/equation?tex=k_%7B%5Ctext%7Beff%7D%7D%20%3D%20%5Cmin%28k%2C%2064%29%5C%5C" alt="k_{\text{eff}} = \min(k, 64)\\" class="ee_img tr_noresize" eeimg="1">

之所以是 `64`，是因为 `SM90` 的 shared memory 仍然可以按 `32` 个 bank、每个 bank `4B` 来理解，一整条 bank line 一共是

<img src="https://www.zhihu.com/equation?tex=32%20%5Ctimes%204%5Ctext%7BB%7D%20%3D%20128%5Ctext%7BB%7D%5C%5C" alt="32 \times 4\text{B} = 128\text{B}\\" class="ee_img tr_noresize" eeimg="1">

对于 `Float16`，一个元素是 `2B`，所以一整条 `128B` bank line 最多容纳

<img src="https://www.zhihu.com/equation?tex=128%5Ctext%7BB%7D%20/%202%5Ctext%7BB%7D%20%3D%2064%5C%5C" alt="128\text{B} / 2\text{B} = 64\\" class="ee_img tr_noresize" eeimg="1">
...
$$
\text{row_bits} = k_{\text{eff}} \times \text{dtype.width}

$$

1.  再用 `copy_bits` 去看“这一行里会出现多少个独立的 copy 向量”：

$$
\text{num_vec} = \frac{\text{row_bits}}{\text{copy_bits}}

$$

1.  最后取对数，得到需要多少个 swizzle bit 才能区分这些向量块：

$$
\text{swizzle_bits} = \log_2(\text{num_vec})
...
<img src="https://www.zhihu.com/equation?tex=128%5Ctext%7BB%7D%20/%2016%5Ctext%7BB%7D%20%3D%208%20%3D%202%5E3%5C%5C" alt="128\text{B} / 16\text{B} = 8 = 2^3\\" class="ee_img tr_noresize" eeimg="1">
...
$$
\text{layout_atom_outer} = (8,32):(32,1)

$$

这张图展示了尚未加入 swizzle 时的 `8x32` layout atom。

![The outer `8x32` layout atom is the unswizzled shared-memory tile.](https://gitee.com/henryzhao28/blog/raw/main-md2zhihu-asset/07_swizzle/83c400ee577e0726-07_swizzle_layout_atom_outer_8x32.jpg)
...
$$
\text{layout_atom} = \text{swizzle} \circ \text{layout_atom_outer}

$$

对于逻辑坐标 `(r, c)`，原始的 `layout_atom_outer = (8,32):(32,1)` 先给出线性地址：

<img src="https://www.zhihu.com/equation?tex=%5Ctext%7Boffset%7D_%7B%5Ctext%7Bouter%7D%7D%20%3D%2032r%20%2B%20c%5C%5C" alt="\text{offset}_{\text{outer}} = 32r + c\\" class="ee_img tr_noresize" eeimg="1">

然后 `S<2,3,3>` 再把这个 offset 映射成新的 shared-memory 地址。逻辑上的 `(8,32)` 形状完全没有变，变化的是“同一个逻辑坐标最终落到哪一个物理 offset 上”。

这里最终使用的是：

<img src="https://www.zhihu.com/equation?tex=S%3C2%2C3%2C3%5Cgt%5C%5C" alt="S<2,3,3\gt\\" class="ee_img tr_noresize" eeimg="1">
...
<img src="https://www.zhihu.com/equation?tex=128%5Ctext%7B%20bit%7D%20%3D%2016%5Ctext%7BB%7D%5C%5C" alt="128\text{ bit} = 16\text{B}\\" class="ee_img tr_noresize" eeimg="1">

而每个 `Float16` 元素占 `2B`，所以一个 copy 向量里有：

<img src="https://www.zhihu.com/equation?tex=16%5Ctext%7BB%7D%20/%202%5Ctext%7BB%7D%20%3D%208%5C%5C" alt="16\text{B} / 2\text{B} = 8\\" class="ee_img tr_noresize" eeimg="1">

个连续元素，也就是正好需要 `3` 个 bit 来标识：

<img src="https://www.zhihu.com/equation?tex=%5Clog_2%208%20%3D%203%5C%5C" alt="\log_2 8 = 3\\" class="ee_img tr_noresize" eeimg="1">
...
$$
\text{new_vec} = \text{vec} \oplus (r \gg 1)
...
$$
\text{After swizzle, layout_atom} = S<2,3,3> \circ 0 \circ (8,32):(32,1)
...
![The `8x32` layout atom after swizzle reflects a bank-conflict-aware shared-memory arrangement.](https://gitee.com/henryzhao28/blog/raw/main-md2zhihu-asset/07_swizzle/3061b267f7ce1299-07_swizzle_layout_atom_8x32.jpg)

---

## `tile_to_shape`
：把一个 swizzled atom 平铺成更大的 shared memory layout
...
$$
\text{after tile_to_shape, layout}
...
![The tiled `128x32` shared-memory layout is formed by repeating the swizzled atom over the target shape.](https://gitee.com/henryzhao28/blog/raw/main-md2zhihu-asset/07_swizzle/087665b800f2a973-07_swizzle_layout_tiled_128x32.jpg)
