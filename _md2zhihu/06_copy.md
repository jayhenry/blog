# 动手学 CuTeDSL 06：从 Tensor 赋值到 Copy Atom：
`copy`
、
`cp.async`
 与 
...
1.  再看 `TensorSSA`、`autovec_copy` 与 `CopyUniversalOp` 这几种等价但抽象层次不同的写法；
1.  重点理解 `cp.async`、`TiledCopy`、`partition_S/D` 如何完成 global-to-shared staging；
1.  最后把 `ldmatrix` 作为 Ampere Tensor Core 路线的补充知识。`sgemm.py` 是 FP32 SIMT GEMM，shared-to-register 走的是 `autovec_copy`，不是 `ldmatrix`。
...
$$
\text{layout_src_tv_tiled} = ((2,16),(8,1)):((128,1),(16,0))

$$

这张图展示了 `cp.async` 的完整 `16x16` tiled copy：32 个线程被组织成两列 thread block，每个线程处理一个连续的 `8` 元素向量。

![The `cp.async` tiled copy on `16x16` organizes 32 threads into two column blocks, each carrying one contiguous 8-element vector.](https://gitee.com/henryzhao28/blog/raw/main-md2zhihu-asset/06_copy/d7f021b29252bb9f-06_copy_cpasync_tiled_16x16.jpg)

把源端 TV layout 单独拿出来看，会更容易理解“谁负责哪一个 `16B` 向量”。

![The source TV layout of the `cp.async` tiled copy highlights how the `16x16` tile is partitioned into thread-owned 8-element vectors.](https://gitee.com/henryzhao28/blog/raw/main-md2zhihu-asset/06_copy/53926967f92e1cba-06_copy_cpasync_src_tv_16x16.jpg)
...
1.  在所有线程都会经过的位置等待 copy 完成，并做 block 级同步；
1.  用 `LdMatrix8x8x16bOp` 创建 shared-to-register 的 copy atom；
1.  用 `make_tiled_copy_tv` 或 `make_tiled_copy` 描述 warp 内线程如何从 shared memory 读出 fragment；
1.  对当前线程调用 `get_slice(tidx)`，再通过 `partition_S` 和 `retile` 得到源端 shared-memory 视图和目标端 register 视图，最后 `cute.copy(...)`。
...
![The standard `8x8` `ldmatrix` copy maps shared-memory rows to a register fragment where each thread owns two values.](https://gitee.com/henryzhao28/blog/raw/main-md2zhihu-asset/06_copy/e79638389dc3cbc7-06_copy_ldmatrix_8x8.jpg)
...
![The `.num_matrices=4` `ldmatrix` copy covers a `16x16` tile with one warp, giving each thread a larger 8-value register fragment.](https://gitee.com/henryzhao28/blog/raw/main-md2zhihu-asset/06_copy/f8f0de6f9e1d8f98-06_copy_ldmatrix_num4_16x16.jpg)
