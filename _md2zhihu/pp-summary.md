...
![image.png](https://gitee.com/henryzhao28/blog/raw/main-md2zhihu-asset/pp-summary/c21f90e115645264-image.png)
...
1.  多batch的stage内顺序：在某个stage内，同样的计算单元，小的micro batch序号都在大序号前面。比如在device 2上，计算单元F按序号0-7排列，计算单元B也按序号0-7排列。
1.  单batch的跨stage顺序：对于某1个microbatch，在前向时，后一个stage必须等前一个stage结束，而反向时，前一个stage必须等后一个stage结束。

对于1F1B来说，除了满足上面通用规则，还有特有的规则。可以观察它的稳态阶段的一部分，即图中的红框，规则如下：

1.  FB交错出现：各个stage上，F和B交错出现，即 (F, B) x N
1.  FB序号关系：对于最后一个stage3（即Device 3），前向F 和 反向B 是同1个microbatch，有相同的microbatch id序号。每往前一个stage，前向F的microbatch id比反向B的microbatch id加1，成等差数列。如stage3中是 F4后紧跟B4，记作(F4, B4)。而 stage2中是 (F4, B3)，stage1中是 (F4,B2)，stage0中是 (F4, B1) 。一般地，对于stage i，计算对为  `(Fx, Bx-(p-1-i))`
...
由此也可以算出1F1B占用的峰值显存。假设一个microbatch中间结果占用的显存为 <img src="https://www.zhihu.com/equation?tex=M_%7BB%7D" alt="M_{B}" class="ee_img tr_noresize" eeimg="1">，那么1F1B在stage0上占用的显存最多，其峰值显存为 <img src="https://www.zhihu.com/equation?tex=pM_B" alt="pM_B" class="ee_img tr_noresize" eeimg="1">。

如果不做流水线并行，一个microbatch中间结果占用显存为 <img src="https://www.zhihu.com/equation?tex=M_%7BB%7D%5E%7Ball%7D" alt="M_{B}^{all}" class="ee_img tr_noresize" eeimg="1">，那么做1F1B流水线并行后，中间结果的峰值显存会变小吗？

答案是不会变小。这是因为虽然切分layer后，一个mircobatch中间结果变为原来的 <img src="https://www.zhihu.com/equation?tex=%5Cfrac%7B1%7D%7Bp%7D" alt="\frac{1}{p}" class="ee_img tr_noresize" eeimg="1">，即 <img src="https://www.zhihu.com/equation?tex=M_%7BB%7D%3DM_%7BB%7D%5E%7Ball%7D%20/%20p" alt="M_{B}=M_{B}^{all} / p" class="ee_img tr_noresize" eeimg="1">。但是像前面说明的，1F1B为了减少气泡同时驻留了p个microbatch的中间结果，所以总的中间结果显存和原来相同，仍为<img src="https://www.zhihu.com/equation?tex=p%2AM_B%3DM_B%5E%7Ball%7D" alt="p*M_B=M_B^{all}" class="ee_img tr_noresize" eeimg="1">。
...
![image.png](https://gitee.com/henryzhao28/blog/raw/main-md2zhihu-asset/pp-summary/939e4199f2e72129-image_1.png)

但是我们只关注 micro id=4~11的8个micro batch，如下。

![image.png](https://gitee.com/henryzhao28/blog/raw/main-md2zhihu-asset/pp-summary/e2e0ec2573c545ed-image_2.png)

然后剔除不属于4~11范围的microbatch

![image.png](https://gitee.com/henryzhao28/blog/raw/main-md2zhihu-asset/pp-summary/3f741505d377585c-image_3.png)
...
-   另外，在stage0调度时，同样保证最多驻留p个F，这样与1F1B有相同的峰值显存 <img src="https://www.zhihu.com/equation?tex=pM_B" alt="pM_B" class="ee_img tr_noresize" eeimg="1">。

通用规则，与1F1B类似，增加一条W规则是：W只依赖同stage内对应的B。

p(pipeline parallel)=4, m(micro batch number)=8的ZB-H1如下

![image.png](https://gitee.com/henryzhao28/blog/raw/main-md2zhihu-asset/pp-summary/5437bb546b698fd6-image_4.png)

ZB-H1稳态阶段的特有规则：

1.  FBW交错
1.  序号关系：对于最后一个stage3为 `(F4, B4, W1)`，stage2为`（F4, B3, W1）`，stage1为`（F4, B2, W1）`，stage0为`（F4, B1, W1）`。即stage i 上计算对为 `(Fx, Bx-(p-1-i), Wx-(p-1))`。其中B的序号随变量stage i成等差序列。
...
![image.png](https://gitee.com/henryzhao28/blog/raw/main-md2zhihu-asset/pp-summary/3ba4e0e1410fc168-image_5.png)

![ZB-H1-extend.png](https://gitee.com/henryzhao28/blog/raw/main-md2zhihu-asset/pp-summary/b20159ae7e21da73-ZB-H1-extend.png)
...
![image.png](https://gitee.com/henryzhao28/blog/raw/main-md2zhihu-asset/pp-summary/17db00125f2844ce-image_6.png)
...
![image.png](https://gitee.com/henryzhao28/blog/raw/main-md2zhihu-asset/pp-summary/81d22244f9a8ade3-image_7.png)

ZB-H2与ZB-H1都做了BW的分离，而他们核心的不同是：在warmup阶段，不受显存的约束，尽可能地调度更多的F，从而减少气泡。实际上可以计算，中间激活值 显存占用约为 1F1B的2倍，即 <img src="https://www.zhihu.com/equation?tex=2pM_B" alt="2pM_B" class="ee_img tr_noresize" eeimg="1">，这也是命名H2的原因。
...
![image.png](https://gitee.com/henryzhao28/blog/raw/main-md2zhihu-asset/pp-summary/3062ad96e72a9d47-image_8.png)
...
![image.png](https://gitee.com/henryzhao28/blog/raw/main-md2zhihu-asset/pp-summary/2191d44f5919d81e-image_9.png)
...
1.  多batch的stage内顺序：在某个stage内，同样的计算单元model chunk，小的micro batch序号都在大序号前面。比如model chunk0在device 2上，计算单元F按序号0-9排列，计算单元B也按序号0-9排列。
1.  单batch的跨stage顺序：对于某1个model chunk的某1个microbatch，在前向时，后一个stage必须等前一个stage结束，而反向时，前一个stage必须等后一个stage结束。其中model chunk0和chunk1的前后stage定义正好相反
    1.  对于正向顺序放置的model chunk0，stage i的前一个stage为 stage i-1，后一个stage为 stage i+1
    1.  对于反向顺序放置的model chunk1，stage i的前一个stage为stage i+1，后一个stage为 stage i-1
...
1.  FB序号关系：看上图，稳态中挑选`F0_8`（0表示modelchunk0，8表示micro batch id为8）所在的计算对，有
    1.  stage0中为 `(F0_8, B1_4, F1_5, B0_1)`
    1.  stage1中为 `(F0_8, B1_4, F1_6, B0_2)`
    1.  stage2中为 `(F0_8, B1_4, F1_7, B0_3)`
    1.  stage3中为 `(F0_8, B1_4, F1_8, B0_4)`
    1.  stage4中为 `(F0_8, B1_4, F1_9, B0_5)`
    1.  stage5~7由于micro batch number太小，超出了稳态范围，进入cooldown状态
    1.  由于DualPipe上下对称，我们只分析上半部分，下半部分对称得到相关结果即可。
    1.  可以看到F0和B1的序号固定相差 p/2=4，而F1和B0则随stage i成等差数列。根据等差数据的某一项以及公差，很容易推导出，stage i中序号关系是  `(F0_x, B1_(x-p/2), F1_(x-p/2+1+i), B0_(x-p+1+i))`

1.  F的起始限制：仍然只考虑上半部分，由于 F1 从下半部分往上传递，较晚到达上层，即在起始warmup阶段，`F1_0`要滞后`F0_0`一段时间。具体地，对于stage i，micro id=0的`F1_0`要晚于`F0_0`的时间为 `p-1-2i` 个单位时间。
...
![image.png](https://gitee.com/henryzhao28/blog/raw/main-md2zhihu-asset/pp-summary/53855841d173ec0d-image_10.png)
...
![image.png](https://gitee.com/henryzhao28/blog/raw/main-md2zhihu-asset/pp-summary/28a85d190300d594-image_11.png)
...
![image.png](https://gitee.com/henryzhao28/blog/raw/main-md2zhihu-asset/pp-summary/d9f508199bfcd35a-image_14.png)

在不考虑zero bubble作者提出的绕过优化器同步的策略时，上面这个调度，和下面是等价的

![image.png](https://gitee.com/henryzhao28/blog/raw/main-md2zhihu-asset/pp-summary/8c1f1e7d1dfce049-image_15.png)

而这个调度和作者提出的ZB-V调度虽略有不同，但非常相似。

![image.png](https://gitee.com/henryzhao28/blog/raw/main-md2zhihu-asset/pp-summary/eb255cdf658a0968-image_16.png)
