...
CuTe DSL 的抽象层次比较高，很多概念只看公式和代码不够直观。因此，本文会尽量配合图示说明：先把每个布局、坐标映射和线程分工画出来，再对应到代码实现。

另外，CuTe DSL中除了底层API外，还有各种各样的上层封装，文档也不够齐全，所以初学很容易陷入到繁杂的“语法糖”高层API中。因此本文重点在讲解底层的API体系，以及高层API和底层API的关联，从而更好地建立使用CuTe DSL API的直觉。

希望通过上面的努力，能更容易理解单个 API 的行为，也能把后续 GEMM 实现中用到的基础知识串成一个完整体系。
...
![shape (8,8), stride (8,1)](img/01_layout_8x8_stride_8_1.jpg)
...
![shape (8,8), stride (10,1)](img/01_layout_8x8_stride_10_1.jpg)
...
![(2,4) 默认紧凑布局](img/02_col_major_2x4.jpg)
