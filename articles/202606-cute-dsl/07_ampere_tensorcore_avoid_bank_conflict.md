...
```mermaid
flowchart LR
    A["global memory<br/>A/B 连续 128-bit 读取"] --> B["cp.async"]
    B --> C["shared memory<br/>sA/sB swizzled layout"]
    C --> D["ldmatrix.x4"]
    D --> E["register fragment"]
    E --> F["tensor core mma"]
```
...
```mermaid
flowchart TD
    A["8 个 FP16<br/>16B copy atom"] --> B["MBase=3<br/>内部连续性不变"]
    B --> C["swizzle_bits<br/>重排多个 16B 块"]
    C --> D["减少同一 warp 指令内<br/>不同地址打到同一 bank 的概率"]
```
...
```mermaid
flowchart TD
    A["M = 3"] --> B["保留 8 个 FP16<br/>也就是 16B 向量内部顺序"]
    C["B = swizzle_bits"] --> D["决定有多少个 16B 向量块<br/>参与 xor 打散"]
    E["S = 3"] --> F["把 128B bank 周期相关高位<br/>xor 到 16B 向量块编号"]
```
...
```mermaid
flowchart TD
    A["逻辑坐标<br/>(m,k) / (n,k)"] --> B["sA/sB layout"]
    B --> C["swizzled physical offset"]
    C --> D["cp.async 写入 shared"]
    C --> E["ldmatrix 从 shared 读取"]
```
