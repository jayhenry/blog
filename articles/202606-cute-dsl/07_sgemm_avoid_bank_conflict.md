...
TODO：解释global memory访问合并和transaction等相关概念。
...
TODO：上面计算有误，应该是 `4*128*3`?

对 A/B 合计也只有 `192 float = 768B`。

## Swizzle 原理

TODO：下面的offset是否可以替换成index？哪一个更专业？这样可以和 articles/202606-cute-dsl/02_hierarchical_layout.md 中索引映射对应。替换成 index 后，可以额外标注，很多文档也称为 offset。
...
TODO：形象地理解中，B到底表示列编号，还是行编号，取决于layout本身是列主序还是行主序。B表示的都是主序的那个维度。
...
表中的 `offset` 是 shared memory 的 element index；FP32 时 `bank = offset % 32`。`S<3,2,5>` 后的 `offset` 与上方 cute-viz 图中的单元格数字一致。


| lane 分组                | 访问坐标`(major,k)` | offset 分布                     | bank 分布         | 结果                     |
| -------------------------- | --------------------- | --------------------------------- | ------------------- | -------------------------- |
...
TODO：上面分析的是 global->shared 时不会有 conflict，补充 shared->register 时是否会有 conflict，做了swizzle前肯定因为沿 M/N 方向顺序排布肯定不会有冲突，但是做了 swizzle 后是否会有？
...
表中的 `offset` 是 shared memory 的 element index；FP32 时 `bank = offset % 32`。`S<3,2,3>` 后的 `offset` 与上方 cute-viz 图中的单元格数字一致。


| lane 分组                | 访问坐标`(major_inner,k)` | offset 分布                  | bank 分布         | 结果                     |
| -------------------------- | --------------------------- | ------------------------------ | ------------------- | -------------------------- |
...
8. [ampere/sgemm.py](https://github.com/nvidia/cutlass/blob/main/examples/python/CuTeDSL/cute/ampere/kernel/dense_gemm/sgemm.py)：padding 版本。
