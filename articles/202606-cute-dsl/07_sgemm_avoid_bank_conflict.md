# 动手学 CuteDSL 07: Shared Memory Bank Conflict 和 Swizzle
...
用一个 `8x8` toy example 更直观。假设 shared memory 也简化成 `8` 个 bank，每个 bank 每次可以服务 `1` 个元素：
...
对于上面这个行主序并且 B=S 的例子，也可以更形象地理解：最低 `2^M` 个元素组成一个 cell；接下来的 `S` 个 bit 表示 cell 二维排布的列编号；接下来 `B` 个 bit 表示行编号，该行编号将和列编号做异或得到新的列编号。具体可以看 [7] 中的示意图和解释。
