...
TODO: 补充 copy bits=128 bits 为最小copy向量的硬件原因。并找官方文档确认这是否对global memory 和 shared memory都生效并且都是128.

## 问题来源

TODO: 精简这个小节，给出典型的尺寸。先分析为什么 row-major 需要处理，col-major 不需要。分析时，重点阐述 global memory 写入 shared memory 和 shared memory读取到 register 两个方面，以及每个方面输入和输出的主序分别是怎样。然后指出后面的方案是对 row-major的处理。
...
TODO: 以具体的尺寸说明为什么这里加4个float padding。在说明中用表格罗列一次完整的warp写入shared memory在无padding和有padding分别是什么情况，来说明如何避免bank conflict。在分析中指出，重点是分析一个warp的行为，当一个warp的读写量大于32个bank即128B时，则只考虑分为多批，每次一批128B的行为。
...
TODO：先介绍 Swizzle 的数学原理，参考 articles/202606-cute-dsl/07_swizzle.md 中 “官方文档[3]里的图可以概括成” 相关片段和官网内容。再介绍Flat Swizzle的具体方案。
