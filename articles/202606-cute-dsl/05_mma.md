...
TODO：补充 mma atom 简单扩展成 tiled mma，tiled mma和它尺寸一致。
...
- CUDA core 路线中，fragment 是当前线程私有的 A/B/C 小块，用来喂给后面的标量 FMA 序列。
 ...
 `partition_A/B/C` 是把一个 CTA 级别的 GEMM tile，系统地切成每个线程要加载、累加、写回的那部分数据。

回到前面的 $128 \times 128$ CTA tile。`tiled_mma`(`permutation_mnk`)只给出一个 $64 \times 64$ 的基础图样，而`partition_C(gC)`会把完整 CTA tile 继续切成 $2 \times 2$ 个这样的图样块。用`@cute.jit` 打印 thread 0 的 C 分区，可以看到：

```text
tCgC = tensor<(0,0) o (1,(4,2),(4,2)):(0,(1@0,64@0),(1@1,64@1))>
```

这里的两个 `(4,2)` 分别对应 M/N 方向，可以读成：
...
两者都会在最终生成的代码里形成多次 FMA，但它们处在不同层级。

- `permutation_mnk` 里的 $4 \times 4$ 属于`TiledMMA`内部的基础图样展开，决定一次`cute.gemm(tiled_mma, ...)` 中每个线程的寄存器 fragment 形状和局部 FMA 展开顺序；
- `partition_C` 产生的 $2 \times 2$ 是外部 CTA tile 相对基础图样的重复 mode，`cute.gemm` 会遍历这些 mode，把同一个基础图样应用到 CTA tile 的不同位置。

所以可以直观理解为：内部的 $4 \times 4$ 决定“一个图样里每个线程算哪些连续元素”，外部的 $2 \times 2$  决定“这个图样在 $128 \times 128$ tile 上重复几块”。
