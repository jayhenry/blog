...
本文用一个 `16x16` 的 `Float16` 矩阵，顺着这个层次走一遍：

1. 先看为什么“标量赋值”能工作，而“slice 赋值”会报错；
2. 再看 `TensorSSA`、`autovec_copy` 与 `CopyUniversalOp` 这几种等价但抽象层次不同的写法；
3. 最后看 `cp.async` 的 global-to-shared staging，以及 `ldmatrix` 的 shared-to-register 装载。
...
TODO：补充异步copy引入的初衷和实际使用背景是为了做copy和计算重叠。

第一，`cp.async` 的 atom 粒度是 `128 bit = 16 B`。对 `Float16` 来说，这正好是 `8` 个元素，而不是一整行 `16` 个元素。

第二，从这段输出可以推断，直接把 `(16)` 的整行 slice 喂给一个 `128-bit` 的 atom，并不能自然表达“整行搬运”这件事：共享内存打印结果里，首行只有前半段是确定写好的，剩余位置没有被这一条 atom 完整覆盖。
...
`cp.async` 负责的是 **GMem -> SMem**。如果接下来要喂给 Tensor Core，通常还要再做一步 **SMem -> Register**，在Ampere架构下配合TensorCore使用的就是 `ldmatrix` [7]。

TODO：说明 ldmatrix 以下章节可跳过，这是 Ampere 架构常用的方法，在后续 Hopper 等架构下已经用新的方式代替。

TODO：说明 ldmatrix 的基本流程。

### 一个容易踩中的同步错误

TODO：把这一小节放到最后附录

先说结论：`cute.arch.sync_threads()` 不能放在依赖 `tidx < M` 这类分支条件的不同控制流里，否则非常容易挂住。
...
下面这段就是一个会挂住的错误实现。问题不在 `cp.async` 或 `ldmatrix` 本身，而在于 `sync_threads()` 被放进了两个分支里：
...
def kernel_func(mA: cute.Tensor, mB: cute.Tensor):
    ...
    if tidx < M:
        ...
        cute.arch.sync_threads()  # 错误：把 block 级 barrier 放进 `if` 分支，结构上已经不安全
        if tidx == 0:
            cute.print_tensor(smem_tensor)
    else:
        cute.arch.sync_threads()  # 错误：`else` 里再放一个 barrier，也不能保证和上面的 barrier 是“同一个同步点”
...
这背后其实是 GPU 硬件执行模型的约束。更准确地说，是 **同一个 warp 内的线程会按同一条指令流推进**：即使某些线程不满足 `if tidx < M`，它们也不是“完全不参与这段控制流”，而是会跟着 warp 一起经历分支发散与 reconvergence，只是在自己不活跃的那一支上不真正执行对应的访存或算术操作。

因此，不能把它想成 CPU 上“不同线程各跑各的 if/else”。对 warp 来说，分支两侧通常是按硬件规则串行化处理的；而 `cute.arch.sync_threads()` 对应的是 block 级 barrier，它要求所有未退出线程都从**同一个程序点**到达同步位置。只要 barrier 被放进分支里，就很容易出现：一部分线程在一条控制流里等待，另一部分线程在另一条控制流里前进，最终整个 block 卡死。
...
围绕这组 copy 例子，可以把 CuTe DSL 里的层次关系压缩成四句话：
...
- `ldmatrix` 适合接在 shared-memory staging 之后使用，而 block 级同步必须放在所有线程都会一致经过的位置。

如果继续往下看 MMA、pipeline 或 shared-memory swizzle，这一篇里的 copy 路径正好就是它们的前置动作：先把 global memory 数据分块搬进 shared memory，再按 warp 级寄存器 fragment 的格式装起来。
